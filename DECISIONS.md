...
## Decision 2026-06-12-03 - Promote The Stdlib App To A Clinician Review Console

| ID | Date | Category | Decision | Rationale | Alternatives Considered | Status |
|---|---|---|---|---|---|---|
| 2026-06-12-03 | 2026-06-12 | UX Scope | Keep the stdlib local app and upgrade it into a clinician-review console with sample-case selection, structured intake review, a Methodology Explorer, grouped Ensemble Contributions, node-level reviewer actions, summary-first results, deeper-dive navigation, and an OpenRouter comparison-only panel. | The current app was functional but too table/raw-JSON oriented for actual reviewer use. The upgrade makes the trust layer easier to inspect while preserving deterministic graph authority and avoiding frontend-framework or production-scope expansion. | Jumping to a full frontend framework was rejected for this milestone because the core need is local demo usability and trust transparency. Making OpenRouter authoritative was rejected because current validation only supports comparison. Keeping raw JSON as the main editing interface was rejected because it is not a realistic clinician-review workflow. | Accepted |

## Decision 2026-06-12-04 - Remediate Browser UX With Explicit Input Precedence And Scoped Artifact Links

| ID | Date | Category | Decision | Rationale | Alternatives Considered | Status |
|---|---|---|---|---|---|---|
| 2026-06-12-04 | 2026-06-12 | Browser UX | For `browser_ux_remediation_v1`, keep the stdlib local app but make input precedence explicit: use the selected sample case as the starting point, let edited pasted text take precedence over the sample, and use uploaded files when the text box is empty. Also serve result Receipt JSON and Receipt Markdown through a read-only `/artifacts/` route scoped to generated local demo receipt files. | Chrome DevTools verification found that browser-normalized default textarea line endings could cause a selected sample to be ignored, and receipt links rendered but returned `404`. Normalized default-text detection fixes the sample path without adding a frontend framework. A scoped artifact route makes receipt links reviewable in the live app while preserving local-only boundaries. | Adding a full frontend framework was rejected as too broad for this remediation. Serving arbitrary workspace paths was rejected as unsafe. Leaving receipt links as static-file-only paths was rejected because it failed live browser review. Making upload override non-empty text was deferred because the current documented behavior is explicit: uploads are used when the text box is empty. | Accepted |
